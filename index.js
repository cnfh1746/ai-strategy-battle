...
class UniversalGameEngine {
    ...
    async startGame() {
        ...
        // 添加玩家名单（不插入游戏规则，游戏规则由GM角色卡定义）
        const playerList = Object.values(this.apiConfigs).map(c => c.name).join('、');
        
        toastr.info('游戏开始！扩展将协调AI行动', 'AI对战');
        
        // 触发GM开始游戏（GM根据自己的角色卡来决定玩什么游戏）
        const opening = await this.callGM(`游戏开始！参与的AI玩家有：${playerList}

请你根据角色卡设定的游戏规则，开始主持游戏。

提醒：使用【轮到：玩家名】让某个AI行动，使用【秘密指示：玩家名|内容】发送秘密信息，说"游戏结束"来结束游戏。`);
// AI策略对战扩展 - 通用版
import { extension_settings, getContext } from "../../../extensions.js";
import { saveSettingsDebounced } from "../../../../script.js";
import { eventSource, event_types } from "../../../../script.js";
